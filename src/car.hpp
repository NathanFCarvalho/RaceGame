...
struct car_structure {
	...
	float hitbox_margin = 0.3f;
    float collision_half_length = std::max(length/2, wheel_forward_offset + wheel_radius) + hitbox_margin;
    float collision_half_width = std::max(length/2, wheel_side_offset + wheel_tire_half_width) + hitbox_margin;
...
struct adversary_car : car {
	float lookahead_distance = 8.0f;
	float heading_gain = 1.4f;
    float adversary_target_speed = 50.0f;
